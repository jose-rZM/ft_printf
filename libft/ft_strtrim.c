...
static unsigned int	contains(char const *s, char c)
{
	while (*s && *s != c)
		++s;
	return (*s == c);
...
static size_t	count_leftmost_ocurrences(char const *s1, char const *set)
{
	size_t	cnt;
...
static size_t	count_rightmost_ocurrences(char const *s1, char const *set)
{
	size_t	cnt;
	size_t	len;
	int		i;
